...
BEGIN
  ...
  O_DEBUG_POINT :='1';
    ...
    ELSE
      SELECT DISTINCT TMP.BANKNAME INTO V_BANK_NAME FROM TC_ME_PARAMETER TMP JOIN TC_TID_HWSR_MAPPING THM ON TMP.MID = THM.MID 
      WHERE THM.HWSRNO = I_TERMINAL_SERIAL_NUMBER;
    END IF;
  
  OPEN C_BANK_INFO FOR
    SELECT * FROM TCH_BANK_MASTER WHERE B_BANKNAME = V_BANK_NAME;
