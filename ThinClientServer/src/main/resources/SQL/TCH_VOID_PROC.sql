...
-- EXTRA CHECK CONDITION FOR IRCTC TO AVOID VOID OF INV NUMBER PREVIOUS OR FUTURE
  ...
  SELECT SUM(FINAL_COUNT) INTO V_COUNT FROM(
  
    SELECT COUNT(1) AS FINAL_COUNT FROM TCH_PAYMENT_TXN TPT 
    ...
    SELECT COUNT(1) AS FINAL_COUNT FROM TCH_SETTLED_PAYMENT_TXN TSP 
      WHERE TSP.P_INVOICENUMBER = I_INV_NUMBER 
      AND TSP.P_TERMINAL_SERIAL_NUMBER = I_TERMINAL_SERIAL_NUMBER  
      AND TSP.P_REQUEST_TYPE = 'VOID'
      AND TSP.P_RESPONSE_CODE = '00'
      ...
      IF V_COUNT = 1 THEN
        BEGIN
          OPEN C_VOID_DATA FOR
            ...
            SELECT * FROM TCH_SETTLED_PAYMENT_RT_VT_TT TSP 
              WHERE TSP.P_INVOICENUMBER = I_INV_NUMBER 
              AND TSP.P_TERMINAL_SERIAL_NUMBER = I_TERMINAL_SERIAL_NUMBER  
              AND TSP.P_REQUEST_TYPE = 'VOID'
              AND TSP.P_RESPONSE_CODE = '00'
  ...
  WHEN NO_DATA_FOUND THEN
    ...
    IF O_DEBUG_POINT = '1AAB' THEN
      O_APP_ERROR_CODE:='V-004';
    END IF;
    IF O_DEBUG_POINT LIKE '1A%' THEN
      O_APP_ERROR_CODE:='V-010';
    END IF;
