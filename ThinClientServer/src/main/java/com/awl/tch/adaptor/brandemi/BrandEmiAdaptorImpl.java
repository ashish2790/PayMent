...
public class BrandEmiAdaptorImpl extends AbstractBrandEmiAdaptor implements BrandEmiAdaptor{
	...
	public void getMenuDetails(SuperMenuObject input) throws TCHServiceException {
		try{
			...
			if(input.getPackectNumber() == null){
				arrMenu = new MenuObject[20];
				input.setMenuObject(lst.subList(0, 20).toArray(arrMenu));
			}else{
				logger.debug("Packet number : " + input.getPackectNumber());
				int startIndex = Integer.parseInt(input.getPackectNumber()) * 20;
				int lastIndex = startIndex + 20;
				logger.debug("startIndex : " + startIndex);
				if(lst.size() > (startIndex + 20)){
					arrMenu = new MenuObject[20];
	...
	public void getSkuCode(SuperMenuObject input) throws TCHServiceException {
			...
			if(Constants.TCH_BRAND_EMI_OFFLINE.equals(validParams.getValidationType())){
				...
				try {
					...
					if(noOfDayswithoutValidation > noOfDaysForMerchant){
						throw new TCHServiceException(ErrorConstants.TCH_BR14, "EXCEED VALIDATION DAYS");
					}
		...
		try{
			if(input.getMenuObject() != null){
				...
				throw new TCHServiceException(ErrorConstants.TCH_BR06, "INVALID SELECTION");
			}
			...
			if(lst != null && !lst.isEmpty())
				...
				logger.info("sku code list is empty..Finding exact reason for the same");
				/*
				 * Check for bin validation. Bin is supported or not
				 */
					logger.debug("Bin checking initiated..");
					if(brandEmiservice.getSKUCodeMenu_WithoutBin(input.getMerchantId(),input)){
						throw new TCHServiceException("BR-07", "BIN NOT SUPPORTED");
					}else{
						throw new TCHServiceException(ErrorConstants.TCH_BR06, ErrorMaster.get(ErrorConstants.TCH_BR06));
					}
				/*
				 * End of changes for Check for bin validation. Bin is supported or not
				 */
			}
		}catch(Exception e){
	...
	public void getTenureDetails(SuperMenuObject input) throws TCHServiceException {
		...
		superObj.setBinNumber(input.getBinNumber());
		logger.debug("BIN value :" + superObj.getBinNumber());
		...
		try {
			...
			if(selectedMenuArr != null && selectedMenuArr.length != 0){
				try{
					...
					logger.debug("Exception while getting validation param :" + e);
					throw new TCHServiceException(ErrorConstants.TCH_BR07, "NO DATA FOUND" );
				}
			}else{
				throw new TCHServiceException(ErrorConstants.TCH_BR03, "INVALID SELECTION");
			}
			...
			if(e1 instanceof TCHQueryException)
				throw new TCHServiceException(ErrorConstants.TCH_BR04, ((TCHQueryException) e1).getErrorMessage());
			else
				throw new TCHServiceException(ErrorConstants.TCH_BR04, ((TCHServiceException) e1).getErrorMessage());
		}
	}
	...
	public void send(SuperMenuObject input) throws TCHServiceException {
		...
		try{
			...
			if(superObj.getMenuObject() != null && superObj.getMenuObject().length != 0){
				...
				throw new TCHServiceException(ErrorConstants.TCH_BR09, "DATA ISSUE");
			}
	...
	private void validateForSerialNumber(ValidationParameter param,final SuperMenuObject input,final SuperMenuObject superObj) throws TCHServiceException, TCHQueryException{
		...
		if(Constants.TCH_BRAND_EMI_REALTIME.equals(param.getValidationType())){
			if(input.getMenuObject() != null){
				...
				throw new TCHServiceException(ErrorConstants.TCH_BR11, "INVALID SKU CODE");
			}
	...
	private void blockSerialNumber(final SuperMenuObject input) throws TCHServiceException, TCHQueryException{
		...
		try {
			...
			throw new TCHServiceException(ErrorConstants.TCH_BR12, "INVALID URL");
		}
		if(response != null){
			if(response.toUpperCase().contains(Constants.TCH_BRAND_TRUE)){
				...
				if(response.toUpperCase().contains(Constants.TCH_BRAND_FALSE)){
					...
					throw new TCHServiceException(ErrorConstants.TCH_BR12, response,"99");
				}
			}
		}
	}
	...
	public void unblockSerialNumber(final SuperMenuObject input) throws TCHServiceException, TCHQueryException{
		...
		try {
			...
			logger.debug("Exception while updating serial number"+e);
			throw new TCHServiceException(ErrorConstants.TCH_BR13, "INTERNAL ERROR");
		}
		if(response != null){
			if(response.toUpperCase().contains(Constants.TCH_BRAND_TRUE)){
				...
				if(response.toUpperCase().contains(Constants.TCH_BRAND_FALSE)){
					logger.debug("Serial number ["+input.getIMIENumber()+"] is not blocked");
					throw new TCHServiceException(ErrorConstants.TCH_BR12, response);
				}
			}
		}
	}
		...
		try {
			...
			if(input.getSkuCodes() == null){
				...
				if(input.getIMIENumber() != null && !input.getIMIENumber().isEmpty()){
					...
					throw new TCHServiceException(ErrorConstants.TCH_BR16, "INVALID INPUT");
				}
			}
		} catch (TCHQueryException e) {
			throw new TCHServiceException(ErrorConstants.TCH_BR15, "RECORD NOT FOUND");
		}
		return input;
	 }
	/*
