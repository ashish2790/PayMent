...
public class RefundServiceImpl extends AbstractQR<Payment> implements RefundService{
	...
	public Payment service(Payment input) throws TCHServiceException {
		...
		String orgTermSerialNumber = null;
		String descFlag = input.getDecisionFlag();
		String terminalIdFromreq = input.getTerminalId();
		/*START: Added API check: by Ganesh Misal */ 
		// check for if the request comes through refund API or terminal
		if (!isApi(input.getTerminalSerialNumber())) {
			orgTermSerialNumber = input.getTerminalSerialNumber().trim();
		...
		/*END: Added API check: by Ganesh Misal */
		...
		if(input.getReferenceValue() != null){
			...
			if(Constants.TCH_REF_API.equals(descFlag) && terminalIdFromreq != null && !terminalIdFromreq.equals(existingDto.getTerminalId())) // changes for IRCTC by Ashish 01/03/2018
				throw new TCHServiceException("R-015", "TID MISMATCH");
			if(!Constants.TCH_REF_API.equals(descFlag)){
			...
			if(Constants.TCH_SUCCESS_RESPONSE.equals(input.getResponseCode())){
				...
				if(Constants.TCH_REF_API.equals(descFlag))
					paymentSettledDto.setRefundFlag(1);
		...
		// make new entry in table for refund 
				...
				if(Constants.TCH_AMEX.equals(paymentDto.getCardLabel())){
					...
					if("Y1".equals(input.getResponseCode())){
					...
					else if(!Constants.TCH_REF_API.equals(descFlag) && Constants.TCH_SUCCESS_RESPONSE.equals(input.getResponseCode())){
						paymentDto.setStatus(Constants.TCH_SUCCESS);
						paymentDto.setRefundFlag(1);
						paymentDto.setRefundStatus(Constants.TCH_SUCCESS);
						payemntRefundDao.save(paymentDto);
						...
						if(Constants.TCH_REF_API.equals(descFlag)){
							logger.debug("record not seaves");
						}else{
	...
	private void calculationForRefund(final PaymentDTO exstDto, final Payment input)throws TCHServiceException{
		...
		if(exstOrgAmount.compareTo(inputOrgAmount) == -1){
			...
			if(Constants.TCH_REF_API.equals(input.getDecisionFlag()))
				exstDto.setOriginalAmount(Double.valueOf(exstOrgAmount.toString())); 
		}
	}
	...
	public Payment serviceFc(Payment input) throws TCHServiceException {
		setProxy(input.getAppName());
		
		try {
		String[] arr = AbstractGenericDao.getMidTids(input.getTerminalSerialNumber(), null);
		input.setMerchantId(arr[0]);
		input.setTerminalId(arr[1]);
	
		logger.info("MID [ "+input.getMerchantId()+" ]");
		logger.info("TID [ "+input.getTerminalId()+" ]");
		logger.debug("Inside serviceFc for refund request");
		
		WalletDTO existingDTO;
		try {
			existingDTO=payemntRefundDao.getExistingDTOFc(input.getReferenceValue(), input.getTerminalSerialNumber());
			input.setRetrivalRefNumber(existingDTO.getServerTxnId());
		} catch (TCHQueryException e) {
			logger.debug("Exception while fetching transaction basis on rrn " + input.getReferenceValue() ,e.getErrorMessage());
			throw new TCHServiceException(e.getErrorCode(), e.getErrorMessage());
		}
		
			if(existingDTO.getTxnAmount().equals(input.getOriginalAmount())){
			input=fcWalletAdaptor.processRefundReq(input);
			if(input.getResponseCode().equals(Constants.TCH_SUCCESS_RESPONSE)){
				if(existingDTO.getIsVoid().equals(Constants.TCH_Y)){
					existingDTO.setIsVoid(null);
				existingDTO.setRefundApproved(Constants.TCH_1);
				walletDaoImpl.update(existingDTO);
				}else{
					/*PaymentSettleDTO settleDto = new PaymentSettleDTO();
					settleDto.setRefundApproved(Constants.TCH_1);
					settleDto.setPaymentId(Long.parseLong(existingDTO.getSettlementPk()));
					settlementDaoImpl.update(settleDto);*/
					payemntRefundDao.updateSettledPayment(input, existingDTO.getTxnRefId(), Constants.TCH_1);
					
				}
			}
		  }
			else{
				throw new TCHServiceException(ErrorConstants.TCH_W104, ErrorMaster.get(ErrorConstants.TCH_W104));
			}
		} catch (TCHServiceException e) {
			logger.error("error while getting refund request :" + e.getErrorMessage());
			throw new TCHServiceException(e.getErrorCode(), e.getErrorMessage());
		} catch (TCHQueryException e) {
	...
	public boolean isApi(String termSerNum) {
		return null == termSerNum  || "".equals(termSerNum);
	}
}
