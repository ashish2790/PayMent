...
public abstract class AbstractServiceImpl {
	 ...
	 */
	public TerminalHardwareParameterDTO checkForPcrFlag(Payment input) throws TCHServiceException{
		// First check for PCR flag is yes or not and then proceed further for actual transaction.
		
		TerminalHardwareParameterDTO t;
		try {
			t = paymentSaleDao.getPCRFlag(input.getTerminalSerialNumber().trim());
			if("Y".equalsIgnoreCase(t.getOpenPcrFlag())){
			...
			logger.debug("Exception in fetching pcr flag :" + e.getMessage());
			throw new TCHServiceException(e.getErrorCode(),e.getErrorMessage(),"99");	// SENDING RESPONSE CODE FOR TERMINAL LEVEL EXCEPTION HANDLING
		}
