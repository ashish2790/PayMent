...
public class SaleCompServiceImpl extends AbstractServiceImpl implements SaleCompService{
	...
	private void calculationForPreAuth(final PaymentAuthDTO preAuthDTO, final Payment input) throws TCHServiceException{
		...
		BigDecimal existingAmount = new BigDecimal(preAuthDTO.getOriginalAmount());
		if(input.getPreauthPer() != null){
			BigDecimal additionalAmomunt  =	existingAmount.multiply(new BigDecimal(input.getPreauthPer())).divide(ONE_HUNDRED);
			existingAmount = existingAmount.add(additionalAmomunt);
		}
		if(existingAmount.longValue() < inputOrgAmount){
